  ...
  const getTestimonials = async () => {
    try {
      const { data } = await axios.get(
        `https://bemyguest-backend.onrender.com/static-content/testimonials`
      );
        ...
        <div className="t-cards-container">
          {testimonialsList.map((item) => (
            <div className="t-card" key={item._id}>
              <img src={item.image} alt="testnomialImage" className="t-image" />
              <div className="t-card-content">
                ...
                <div className="t-card-para">
                  <p className="t-card-para-p">
                    {item.message.length > 0 ? shortner(item.message) : "N/A"}
                  </p>
